...
function result(imageNum = 7) {
  ...
  const kami = (char, num) => {
	...
	if((num-1) % 3 === 0)
	{
		otpt = `<div class="col-12"><div class="row"><div class="col-4 px-0"></div><div class="result kami col-4 pl-0 pr-1 pb-1 d-flex"><div class="flex-fill row mx-0 result-mcontainer p-1 border rounded-lg"><div class="d-flex right col-12 col-xl-10 pl-0 pr-0"><div class="row mx-0"><img class="col-12 px-0 mx-xl-auto border rounded-lg" src="${char.img}"><div class="d-flex col-12 pl-1 pr-0 text-break justify-content-center" title="${charTooltip}">${num}. ${charName}</div></div></div></div></div><div class="col-4 pl-0 pr-1"></div></div></div>`;
	}
	else
	{
		otpt = `<div class="col-12"><div class="row"><div class="col-4 pl-0 pr-1"></div><div class="result kami col-4 pl-0 pr-1 pb-1 d-flex"><div class="flex-fill row mx-0 result-mcontainer p-1 border rounded-lg"><div class="d-flex left col-3 align-items-center justify-content-end px-0"></div><div class="d-flex right col-9 pl-1 pr-0"><div class="row mx-0"><img class="col-12 px-0 mx-xl-auto border rounded-lg" src="${char.img}"><div class="d-flex col-12 pl-1 pr-0 text-break text-center justify-content-center" title="${charTooltip}">${num}. ${charName}</div></div></div></div></div><div class="col-4 pl-0 pr-1"></div></div></div>`;
	}
  ...
  const imgRes = (char, num) => 
  {
	...
	if((num-1) % 3 === 0)
	{
		otpt = `<div class="result image col-4 pl-0 pr-1 pb-1 d-flex"><div class="flex-fill row mx-0 result-mcontainer p-1 border rounded-lg"><div class="d-flex right col-12 col-xl-10 pl-0 pr-0"><div class="row mx-0"><img class="col-12 px-0 mx-xl-auto border rounded-lg" src="${char.img}"><div class="d-flex col-12 pl-1 pr-0 text-break justify-content-center" title="${charTooltip}">${num}. ${charName}</div></div></div></div></div>`;
	}
	else
	{
  ...
  const timeStr = `This sorter was completed on ${new Date(timestamp + timeTaken).toString()} and took ${msToReadableTime(timeTaken)}. <a href="${location.protocol}//${sorterURL}" style="color: #2196F3">Do another sorter?</a>`;
...
function populateOptions() {
  ...
  const optInsert = (name, id, tooltip, checked = true, disabled = false) => {
    return `<div class="col-6 col-md-4 col-xl-3"><label class="checkbox-container" title="${tooltip?tooltip:name}"> ${name}<input id="cb-${id}" name="${id.slice(0, 6)}" type="checkbox" ${checked?'checked':''} ${disabled?'disabled':''} ><span class="checkmark"></span></label></div>`;
  };
  const optInsertLarge = (name, id, tooltip, checked = true) => {
    // return `<div class="col-12 large option"><label class="checkbox-container" title="${tooltip?tooltip:name}"> ${name}<input id="cbgroup-${id}" name="rb" type="checkbox" ${checked?'checked':''}><span class="checkmark"></span></label></div>`;
    return `<p style="color:white">.</p><label class="col-12 form-control justify-content-center" title="${tooltip?tooltip:name}"><input id="cbgroup-${id}" name="rb" type="checkbox" ${checked?'checked':''}/> ${name}</label>`;
  };
    ...
    if ('sub' in opt) {
      ...
      groupbox.parentElement.addEventListener('click', () => {
        opt.sub.forEach((subopt, subindex) => {
          ...
          if (groupbox.checked) { document.getElementById(`cb-${opt.key}-${subindex}`); }
        });
      });
    } else {
